...
# --- PREREQUISITES ---------------------------------------------
# This script calls psql directly as DB_USER. Authentication must
# be handled via one of:
#   a) ~/.pgpass entry:  127.0.0.1:5432:tenable_trends:tenable_trends_user:yourpassword
#   b) PGPASSWORD env var in the cron environment:
#        PGPASSWORD=yourpassword 0 3 * * * /root/tenable-tracker/maintenance.sh
#   c) pg_hba.conf set to "trust" for localhost (not recommended for production)
# Without one of the above, the psql calls below will hang or fail silently.
